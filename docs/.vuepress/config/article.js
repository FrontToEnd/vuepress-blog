const article = [
  {
    title: "项目优化实践",
    collapsable: true,
    children: [
      "optimize/vue_optimize_gzip",
      "optimize/vue_optimize_webpack",
      "optimize/vue_optimize_branch",
      "optimize/vue_optimize_standard",
      "optimize/vue_optimize_commit",
      "optimize/vue_optimize_debounce",
      "optimize/tailwindcss",
      "optimize/Performance",
      "optimize/前端性能优化原理与实践",
    ],
  },
  {
    title: "技术体系",
    collapsable: true,
    children: ["system/css硬件加速", "system/取消请求", "system/SOLID"],
  },
  {
    title: "技术探索",
    collapsable: true,
    children: [
      "explore/zqy_cli",
      "explore/npm",
      "explore/browser_render",
      "explore/browser_userAgent",
      "explore/OSS",
      "explore/sentry",
      "explore/wasm",
      "explore/nvm",
      "explore/jsPlumb",
      "explore/nginx",
      "explore/excel",
      "explore/jest",
      "explore/jest_02",
      "explore/download",
      "explore/JSON",
      "explore/bpmn",
      "explore/原生开发探索",
      "explore/relative_time",
      "explore/deploy",
    ],
  },
  {
    title: "技术总结",
    collapsable: true,
    children: [
      "tech/fe_train",
      "tech/renderer",
      "tech/yunxi",
      "tech/design_document",
      "tech/technology_research",
    ],
  },
  {
    title: "vue相关",
    collapsable: true,
    children: [
      "vue3/vue3.0",
      "vue3/mitt",
      "vue3/Functional_Components",
      "vue3/vite中集成tailwindcss",
    ],
  },
  {
    title: "react相关",
    collapsable: true,
    children: [
      "react/使用TypeScript创建React应用",
      "react/React技巧之设置具有默认值的可选props",
      "react/React技巧之将对象作为props传递给组件",
      "react/React技巧之将函数作为props传递",
      "react/React技巧之将CSS作为props传递",
      "react/React技巧之具有空对象初始值的useState",
      "react/React技巧之将useState作为对象",
      "react/React技巧之将useState作为对象数组",
      "react/React技巧之将useState作为字符串数组",
      "react/React技巧之输入onFocus和onBlur事件",
      "react/React技巧之select标签设置占位符",
      "react/React技巧之用钩子clearTimeout",
      "react/React技巧之获取鼠标坐标位置",
      "react/React技巧之设置target=_blank",
      "react/React技巧之状态更新",
      "react/React技巧之useRef钩子",
      "react/React技巧之在state数组中添加元素",
      "react/React技巧之重定向表单提交",
      "react/React技巧之鼠标悬浮添加行内样式",
      "react/React技巧之有条件地添加属性",
      "react/React技巧之设置data属性",
      "react/React技巧之导入并使用Image",
      "react/React技巧之调用子组件函数",
      "react/React技巧之设置onClick监听器",
      "react/React技巧之检查复选框是否选中",
      "react/React技巧之组件中返回多个元素",
      "react/React技巧之字符串插值",
      "react/React技巧之循环遍历对象",
      "react/React技巧之理解Eslint规则",
      "react/React技巧之移除状态对象中的键",
      "react/React技巧之移除状态数组中的对象",
      "react/React技巧之添加或移除类",
      "react/React技巧之改变元素样式",
      "react/React技巧之使用ref获取元素宽度",
      "react/React技巧之获取元素类名",
      "react/React技巧之处理tab页关闭事件",
      "react/React技巧之检查元素是否可见",
      "react/React技巧之发出http请求",
      "react/React技巧之打开文件输入框",
      "react/React技巧之设置input值",
      "react/React技巧之表单提交获取input值",
      "react/React技巧之导入组件",
      "react/React技巧之中断map循环",
      "react/React技巧之改变SVG的颜色",
    ],
  },
  {
    title: "CSS",
    collapsable: true,
    children: [
      "css/filter和backdrop-filter",
      "css/css_property",
      "css/PostCSS",
      "css/CSS隐藏元素",
    ],
  },
  {
    title: "代码片段",
    collapsable: true,
    children: [
      "code/code_fragment",
      "code/sass",
      "code/verify_utils",
      "code/ramda",
      "code/不规则图形",
    ],
  },
  {
    title: "文章翻译",
    collapsable: true,
    children: [
      "translation/如何处理TypeScript中的可选项和Undefined",
      "translation/使用Vue3 + Vite + Pinia创建SPA",
    ],
  },
  {
    title: "资源收集",
    collapsable: true,
    children: [
      "resource/resource",
      "resource/2021前端必读",
      "resource/web站点",
    ],
  },
  {
    title: "杂记",
    collapsable: true,
    children: ["notes/OTC", "notes/MAC", "notes/time"],
  },
];

module.exports = {
  article,
};
